...
### 2026-04-19 — y-flip-tta-violet-e20
- **Hypothesis:** Training used Y-flip augmentation (50% of batches flipped along Y) — so the model is Y-equivariant by training. At inference we only evaluate on the original orientation. Averaging `model(x)` and `un-flip(model(flip(x)))` should (a) symmetrize predictions around Y=0 (the airflow really is approximately Y-symmetric for F1 centerline wake), and (b) reduce single-pass noise via the 2-sample ensemble. Pure inference change — no retraining.
- **Change:** `predict.py` and `train.py::validate()` — wrap single forward pass in a 2-sample average: `pred = (model(v_in, pos) + flipY(model(flipY(v_in), flipY(pos)))) / 2`. Flip = multiply Y-component by -1 for both position and velocity; un-flip predictions by the same trick.
- **Result:** val/l2 = **0.9419** (no retraining; E19 checkpoint + TTA). Δ = **-0.0128 (-1.3%)** vs E19 non-TTA 0.9547. Leaderboard: **rank 8 → rank 6**, passed fern (0.9433) and askeladd (0.9480). Inference cost: 2× forward passes (~2× validation time).
- **Verdict:** kept — cheap win, low risk, immediate.
- **Notes:** Per-component MAE improved across the board: Ux 0.6335→0.6266, Uy 0.3063→0.2968 (largest relative drop, 3.1%), Uz 0.4571→0.4390. Y-flip TTA helps Uy most as expected (symmetry axis). Remaining gap to leader thorfinn=0.685: 0.26, still 28% off. **Next ideas**: (a) combine TTA with a scale-up (hidden=384, 7+ edge blocks) and longer training (300 ep), (b) 4-way TTA (identity / Y-flip / X-flip / XY-flip — though X symmetry may not hold for wake directionality), (c) per-timestep output heads so each of the 5 predicted frames gets its own small decoder, (d) Chamfer/repulsion loss for the masked airfoil points. The TTA result also suggests further symmetrization could help.

### 2026-04-19 — residual-head-violet-e19
- **Hypothesis:** The baseline model had to learn absolute velocity from scratch at every point. Reframe output as `v_in[:, -1] + delta` so the baseline prediction is "flow continues as-is" (a strong physical prior: on 5-step timescales, most of the field is laminar and close to the last input frame). Model learns only the evolution delta, which has smaller magnitude and smoother structure → better gradient dynamics, faster descent. Zero-init point_head so at step 0 the model outputs *exactly* the last input frame.
- **Change:** `train.py` `BaselineMLP.forward` — after computing `out_combined`, add `v_norm[:, -1:].expand(-1, T_OUT, -1, -1)` in normalized space before denormalizing. Zero-init `point_head[1]` weight & bias. All other hyperparameters identical to E17 (hidden=320, 6 point blocks, 6 edge blocks, 10k fine anchors, 2k coarse anchors, 5-epoch warmup, cosine 200 epochs, lr=1e-3).
- **Result:** val/l2 = **0.9547** (epoch 194). 200 epochs × 102 s = 339 min (just under 360-min budget). train 0.0346 → 0.0103 (normalized). 9.4 GB bf16. Run `dm2lmn36`. Auto-predict saved to `/mnt/new-pvc/predictions/apr16/violet/e91ed26/val.pt`. Commit `2246111` (code), `3fb3569` (ckpt).
- **Verdict:** kept — **-3.0% vs E17** (0.9843 → 0.9547). Cleanest wins in the run. Trajectory more favorable at every checkpoint: e8=1.37, e20=1.16, e43=1.09, e71=1.06, e98=0.99, e131=0.98, e149=0.97, e168=0.96, e194=0.95. Still descending at timeout (val gap between last 20 epochs < 0.003) — convergence nearly flat.
- **Notes:** The trick works as predicted: residual reframing gave early-epoch val/l2 ≈1.4 (vs E17's ~1.67) and the gap persisted to end of training. Train loss also lower at matched epochs (E19 e50=0.0165 vs E17 e50~0.021), indicating optimization is genuinely easier. **Next ideas**: (a) scale further — hidden=384 or extra edge blocks, now that each token only needs to model a delta; (b) stretch training to 300 epochs since loss is still descending; (c) residual from per-point linear extrapolation `v_in[-1] + (v_in[-1]-v_in[-2]) * k` rather than constant; (d) per-timestep decoder (5 separate output heads) — each delta is different anyway; (e) test-time augmentation via Y-flip ensemble, cheap win. Leader at 0.69, me at 0.95; gap = 0.26, still 27% off. Need multi-step gains.
