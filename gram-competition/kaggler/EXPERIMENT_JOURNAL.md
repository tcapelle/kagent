...
- **Result:** v36 solo val/l2 = **0.8608** at ep57 of 90 (GPU contention early → 144s/epoch × 25 epochs, then cleared → 66s/epoch × 32 epochs; 110-min budget cut at ep57 so cosine only reached ~60% anneal). Solo weaker than recent voxel_mid=64 full-cosine (v29 0.8420, v33 0.8471, v28 0.8541). 23-seed full-weighted = 0.7498 (top23_T=0.015), but **top16_T=0.02 = 0.7488** — new best. Gain 0.0017 vs. prior 0.7505. 12.5% under v6 solo.
- **Verdict:** kept. Two effects stacking: (a) voxel_mid=96 architectural diversity adds ~0.0007 ensemble value despite weaker solo; (b) top-k filter drops 7 weakest seeds, adds another ~0.0010. Plus the script was using all 23 seeds at uniform = 0.7543 → weighted 0.7498 → top16_weighted 0.7488 — both steps help.
- **Notes:** top-k grid shows a clear sweet spot at K=15-16 out of 23: trimming beyond drops diversity, keeping more adds noise. Solo l2 of the included seeds: v29 0.8420 through v23 0.8602 — the bar is ~0.860. Next experiments: (v37) *resume/extend v36* to complete the cosine anneal — the model was still descending at cutoff, a few more epochs at near-zero LR could cut solo to ~0.84. Or (v37 alt): **voxel_mid=96 2nd seed** with epochs=60 so full cosine fits in budget — that guarantees full anneal in ~100 min. (v38): try `hidden=384` for another arch axis. If both voxel_mid=96 and hidden=384 contribute, architectural diversity is a new productive axis. Also worth testing: **median ensembling** vs weighted mean — robust to outlier predictions per-point.
