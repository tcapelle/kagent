...
class VoxelFlowNet(nn.Module):
    ...
    def _voxelize(self, v_in_norm, pos, sdf):
        ...
        SDF goes in as mean-only so the CNN sees airfoil proximity structure.
        """
        ...
        sdf_scaled = (sdf / 0.5).unsqueeze(-1)  # [B, N, 1] — same scale as _sdf_enc
        feat_mean_in = torch.cat([v_feat, sdf_scaled, ones], dim=-1)  # [B, N, 17]
