"""Wavg across even older checkpoints by zero-padding missing keys."""
import json
...
def load_and_complete(path, reference_state):
    ...
    for k, ref in reference_state.items():
        ...
        # If shape mismatches (e.g. different grid_size, n_blocks), drop it.
        elif s[k].shape != ref.shape:
            s[k] = torch.zeros_like(ref) if ref.is_floating_point() else ref.clone()
    return s
...
def main():
    ...
    # Full candidate pool: 237-243 key
    candidates = [
        "model-wavg2-266db37",  # exp58 result
        "model-wavg-816156a",   # exp57 result
        ...
        "model-jay6zniz",   # 241
        "model-uesu5tb6",   # 241
        # Earlier 237-key (pre-knn, with out_refine + aux)
        "model-bal6xybc",   # exp33, 237
        "model-18f6e3td",   # exp32, 237
        "model-670v4v75",   # exp31, 237
        "model-fdgxhd3i",   # exp40, 237
        "model-kvptxsnv",   # exp30, 237
        "model-mgo03egs",   # exp38, 237
    ]
    scores = {}
    states = {}
    for c in candidates:
    ...
    pvc = PVC_CKPTS / f"model-wavg3-{commit}"
