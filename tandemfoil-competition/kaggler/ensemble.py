...
Picks up existing per-commit predictions on the PVC, averages them
per-sample per-channel, and writes a new submission keyed on HEAD commit.

Usage: git commit first, then `python ensemble.py --sources <hash1> <hash2> [...] [--weights w1 w2 ...]`.
"""
...
PRED_DIR = Path(f"/mnt/new-pvc/predictions/{RESEARCH_TAG}/tanjiro")
