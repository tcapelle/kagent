...
Loads multiple Transolver checkpoints, runs each on test splits, and
saves the mean prediction per sample to the current git commit path.

Run:
  python ensemble.py --agent askeladd --checkpoints ckpt1.pt ckpt2.pt [...]
"""
...
@dataclass
class Config:
    checkpoints: list[str] = field(default_factory=list)  # multiple .pt paths
...
# Load all models up-front (small enough)
models = []
for ckpt_path in cfg.checkpoints:
    ...
    m.eval()
    models.append(m)
    print(f"Loaded {ckpt_path}  ({sum(p.numel() for p in m.parameters())/1e6:.2f}M)")
...
for split in TEST_SPLITS:
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(test_files), cfg.batch_size), desc=split, leave=False):
            ...
            # Average predictions across models (in normalized space then denormalize)
            accum = None
            for m in models:
                pred_norm = m({"x": x_norm})["preds"]
                accum = pred_norm if accum is None else accum + pred_norm
            pred_norm = accum / len(models)
