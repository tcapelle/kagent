...
Loads model from checkpoint + sibling config.yaml, runs inference on each
test split (4 splits, 200 samples each), saves per-split predictions to PVC.
...
Run:
  python predict.py --checkpoint models/model-<id>/checkpoint.pt --agent frieren
"""
