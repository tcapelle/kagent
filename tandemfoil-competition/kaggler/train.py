...
def make_subsample_collate(n_vol: int):
    """Collate that keeps all surface nodes and subsamples n_vol volume nodes per sample."""
    def _collate(batch):
        new_batch = []
        for x, y, is_surf in batch:
            surf_idx = torch.where(is_surf)[0]
            vol_idx = torch.where(~is_surf)[0]
            if len(vol_idx) > n_vol:
                perm = torch.randperm(len(vol_idx))[:n_vol]
                vol_idx = vol_idx[perm]
            keep = torch.cat([surf_idx, vol_idx])
            new_batch.append((x[keep], y[keep], is_surf[keep]))
...
def main():
    ...
    train_collate = make_subsample_collate(cfg.n_vol_subsample)
    ...
    # per-channel weights for L1 loss: extra weight on pressure channel
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for x, y, is_surface, mask in tqdm(train_loader, desc=f"Epoch {epoch+1}/{MAX_EPOCHS}", leave=False):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                err = (pred - y_norm).abs() * ch_w  # L1, channel-weighted

                vol_mask = mask & ~is_surface
                surf_mask = mask & is_surface
                vol_loss = (err * vol_mask.unsqueeze(-1)).sum() / (vol_mask.sum().clamp(min=1) * 3)
                surf_loss = (err * surf_mask.unsqueeze(-1)).sum() / (surf_mask.sum().clamp(min=1) * 3)
    ...
    if best_metrics:
        ...
        # Mirror best ckpt to PVC + local checkpoints/best.pt
        research_tag = os.environ.get("RESEARCH_TAG", "default")
        agent = cfg.agent or "unknown"
        pvc_dir = Path(f"/mnt/new-pvc/kagent/{research_tag}/{agent}/checkpoints/model-{run.id}")
        ...
        local_best = Path("checkpoints")
        local_best.mkdir(exist_ok=True)
        shutil.copy(model_path, local_best / "best.pt")
        shutil.copy(model_dir / "config.yaml", local_best / "config.yaml")
        print(f"Mirrored best ckpt → {pvc_dir} and checkpoints/best.pt")
