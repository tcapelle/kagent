...
class Config:
    ...
    agent: str | None = None
    debug: bool = False
    amp: bool = True  # bf16 autocast for speed
...
def main():
    ...
    # Best-checkpoint mirror used by the experiment-loop spec.
    mirror_dir = Path("checkpoints")
    mirror_dir.mkdir(exist_ok=True)
    with open(mirror_dir / "config.yaml", "w") as f:
        yaml.dump(model_config, f)
    mirror_path = mirror_dir / "best.pt"
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if mean_val_loss < best_val:
            ...
            torch.save(sd, mirror_path)
            torch.save(sd, pvc_path)
            tag = " *"
