...
class Config:
    ...
    surf_p_weight: float = 2.0  # bumped — leaderboard ranks pressure only
    ema_decay: float = 0.999
    rc_single_boost: float = 10.0  # very strong single emphasis
    epochs: int = 8
...
def main():
    ...
    if cfg.init_checkpoint:
        sd = torch.load(cfg.init_checkpoint, map_location=device, weights_only=True)
        model.load_state_dict(sd)
        print(f"Warm-started from {cfg.init_checkpoint}")
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for x, y, is_surface, mask in tqdm(train_loader, desc=f"Epoch {epoch+1}/{MAX_EPOCHS}", leave=False):
            ...
            huber_err = torch.where(
                abs_err < huber_beta,
                0.5 * sq_err / huber_beta,
                abs_err - 0.5 * huber_beta,
            )
