...
class Config:
    ...
    n_head: int = 6
    slice_num: int = 64
    train_subsample: int = 40000  # 0 = no subsample (full mesh)
    bf16: bool = True
...
def subsample_collate(batch):
    """Random subsample of non-surface nodes for training (keeps all surface)."""
...
warmup_epochs = min(3, MAX_EPOCHS // 4) if not cfg.warm_start else 0
if warmup_epochs > 0:
