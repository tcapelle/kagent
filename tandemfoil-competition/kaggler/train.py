...
def main():
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if mean_val_loss < best_val:
            ...
            # Mirror to git-tracked path and durable PVC per KAGGLER_AGENT contract.
            ckpt_git = Path("checkpoints/best.pt")
            ckpt_git.parent.mkdir(parents=True, exist_ok=True)
            torch.save(model.state_dict(), ckpt_git)
            with open(ckpt_git.parent / "config.yaml", "w") as f:
                yaml.dump(model_config, f)
            pvc_dir = Path(f"/mnt/new-pvc/kagent/{os.environ.get('RESEARCH_TAG','apr23')}/"
                           f"{os.environ.get('KAGGLER_NAME','edward')}/checkpoints/model-{run.id}")
