...
def main():
    ...
    max_timeout = float(os.environ.get("MAX_TIMEOUT_MIN", MAX_TIMEOUT))
    ...
    chan_w = torch.tensor([1.0, 1.0, cfg.p_weight], device=device)  # pressure-emphasised channel weights
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if mean_val_loss < best_val:
            ...
            # Mirror best checkpoint locally for git tracking
            local_ckpt = Path("checkpoints/best.pt")
            local_ckpt.parent.mkdir(parents=True, exist_ok=True)
            torch.save({"state_dict": model.state_dict(), "model_config": model_config}, local_ckpt)
