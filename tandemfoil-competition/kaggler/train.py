...
def apply_no_slip(pred: torch.Tensor, is_surface: torch.Tensor,
                   y_mean: torch.Tensor, y_std: torch.Tensor) -> torch.Tensor:
    """Zero out Ux,Uy predictions on airfoil surface nodes in normalized space."""
    # In normalized space, zero physical velocity corresponds to -y_mean/y_std.
    zero_norm = (-y_mean / y_std)  # [3]
    surf = is_surface.unsqueeze(-1).float()  # [B,N,1]
    # Replace channels 0,1 with zero_norm on surface
    u_target = pred[..., :2].clone()
    u_target = (1 - surf) * pred[..., :2] + surf * zero_norm[:2]
    return torch.cat([u_target, pred[..., 2:3]], dim=-1)
...
def main():
    ...
    # Persistent durable copy on PVC
    ...
    for epoch in range(MAX_EPOCHS):
        if (time.time() - train_start) / 60.0 >= MAX_TIMEOUT - 1.0:
            print(f"Timeout approaching ({MAX_TIMEOUT} min). Stopping.")
            break
        ...
        # Target metric proxy: avg surface pressure MAE across val splits
        ...
        tag = ""
        # Select best checkpoint by surface pressure MAE (the competition metric)
        if avg_surf_p < best_val:
            best_val = avg_surf_p
            best_metrics = {"epoch": epoch + 1, "val_loss": mean_val_loss, "avg_mae_surf_p": avg_surf_p}
            for sm in split_metrics.values():
                best_metrics.update({f"best_{k}": v for k, v in sm.items()})
            torch.save(model.state_dict(), model_path)
            # Durable copy on PVC
            torch.save(model.state_dict(), pvc_ckpt_dir / "checkpoint.pt")
            tag = " *"
        ...
        print(
            ...
            f"val[loss={mean_val_loss:.3f} avg_surf_p={avg_surf_p:.2f}  {split_summary}]{tag}"
        )
