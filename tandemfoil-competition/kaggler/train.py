...
warmup_e = max(0, min(cfg.warmup_epochs, MAX_EPOCHS // 4))
if warmup_e > 0:
    warmup = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=1e-2, total_iters=warmup_e)
    cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max(MAX_EPOCHS - warmup_e, 1))
    scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer, schedulers=[warmup, cosine], milestones=[warmup_e])
else:
...
for epoch in range(MAX_EPOCHS):
    ...
    for x, y, is_surface, mask in tqdm(train_loader, desc=f"Ep {epoch+1}/{MAX_EPOCHS}", leave=False):
    ...
    print(
        f"Ep {epoch+1:3d} ({dt:.0f}s) [{peak_gb:.1f}GB]  "
